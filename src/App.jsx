...
import { Truck, Search, Upload, Trash2, BarChart3, TrendingUp, Calendar, Download } from 'lucide-react';
...
const Dashboard = () => {
  ...
  // Función para limpiar y procesar números en notación científica
  const cleanScientificNotation = (value) => {
    if (!value) return '';
    const str = String(value).trim();
    
    // Si tiene notación científica (ej: 2,30018E+11)
  ...
  // Función para limpiar caracteres especiales (Ã±, Ã', etc)
  ...
  // Función para parsear CSV con limpieza avanzada
  const parseCSV = (text) => {
    ...
    for (let i = headerIndex + 1; i < lines.length; i++) {
      ...
      const fechaLimpia = fechaRaw.split(' ')[0]; // Tomar solo la fecha sin hora
    ...
    Array.from(files).forEach(file => {
      ...
      reader.onload = (e) => {
        try {
          ...
          console.log(`Procesadas ${newRows.length} filas del archivo ${file.name}`);
          
          setData(prev => {
            const combined = [...prev, ...newRows];
            // Eliminar duplicados por guía
  ...
  const exportToCSV = () => {
    ...
    link.download = `inter_rapidisimo_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
